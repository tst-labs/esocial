...
@SpringBootTest
public class GeradorXmlAdmissaoTest {
	
	@Autowired
	GeradorXmlAdmissao gerador;

	@Test
	public void deveGerarXmlAdmissao() throws Exception {
	...
	@Test
	public void deveGerarXmlAdmissaoRetificacao() throws Exception {
		Evento evento = getEvento();
		evento.getOcorrencia().setOperacao(Operacao.INCLUSAO);
		Admissao ocorrencia = (Admissao) evento.getOcorrencia().getDadosOcorrencia();
	...
	private Evento getEvento() throws Exception {
		...
		Ocorrencia ocorrencia = exemplo.lerOcorrencia(TipoOcorrencia.ADMISSAO_TRABALHADOR);
		...
		evento.setTipoEvento(TipoEvento.ADMISSAO_TRABALHADOR);
