...
@SpringBootTest
public class GeradorXmlTSVInicioTest {
	
	@Autowired
	GeradorXmlTSVInicio gerador;

	@Test
	public void deveGerarXmlTSVInicio() throws Exception {
		Evento evento = getEvento();
		evento.getOcorrencia().setOperacao(Operacao.INCLUSAO);
		String xml = gerador.gerarXml(evento);
		assertThat(xml).contains("evtTSVInicio");
		assertThat(xml).contains("<indRetif>1</indRetif>");
	}
	
	@Test
	public void deveGerarXmlTSVInicioRetificacao() throws Exception {
		Evento evento = getEvento();
		evento.getOcorrencia().setOperacao(Operacao.INCLUSAO);
		TSVInicio ocorrencia = (TSVInicio) evento.getOcorrencia().getDadosOcorrencia();
	...
	private Evento getEvento() throws Exception {
		...
		Ocorrencia ocorrencia = exemplo.lerOcorrencia(TipoOcorrencia.TSV_INICIO);
		...
		evento.setTipoEvento(TipoEvento.TSV_INICIO);
