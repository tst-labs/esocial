...
@SpringBootTest
public class GeradorXmlFechaEvPerTest {
	
	@Autowired
	GeradorXmlFechamentoPeriodicos gerador;

	@Test
	public void deveGerarXmlFechamentoPeriodicos() throws Exception {
		Evento evento = getEvento();
		evento.getOcorrencia().setOperacao(Operacao.NORMAL);
		String xml = gerador.gerarXml(evento);
		assertThat(xml).contains("evtFechaEvPer");
	}
	
	private Evento getEvento() throws Exception {
		ExemploOcorrenciaServico exemplo = new ExemploOcorrenciaServico();
		Ocorrencia ocorrencia = exemplo.lerOcorrencia(TipoOcorrencia.FECHAMENTO_PERIODICOS);
		...
		evento.setTipoEvento(TipoEvento.FECHAMENTO_PERIODICOS);
