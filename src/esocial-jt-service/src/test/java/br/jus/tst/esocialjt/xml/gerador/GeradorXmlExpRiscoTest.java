...
@SpringBootTest
public class GeradorXmlExpRiscoTest {
	
	@Autowired
	GeradorXmlExpRisco gerador;

	@Test
	public void deveGerarXmlExpRisco() throws Exception {
		Evento evento = getEvento();
		evento.getOcorrencia().setOperacao(Operacao.NORMAL);
		String xml = gerador.gerarXml(evento);
		assertThat(xml).contains("evtExpRisco");
		assertThat(xml).contains("<indRetif>1</indRetif>");
	}

	@Test
	public void deveGerarXmlExpRiscoRetificacao() throws Exception {
		Evento evento = getEvento();
	...
	private Evento getEvento() throws Exception {
		...
		Ocorrencia ocorrencia = exemplo.lerOcorrencia(TipoOcorrencia.EXP_RISCO);
		...
		evento.setTipoEvento(TipoEvento.EXP_RISCO);
