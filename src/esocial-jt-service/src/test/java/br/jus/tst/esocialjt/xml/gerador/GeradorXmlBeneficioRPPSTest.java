...
@SpringBootTest
public class GeradorXmlBeneficioRPPSTest {
	
	@Autowired
	GeradorXmlBeneficioRPPS gerador;

	@Test
	public void deveGerarXmlBenPrRP() throws Exception {
		Evento evento = getEvento();
		evento.getOcorrencia().setOperacao(Operacao.INCLUSAO);
		String xml = gerador.gerarXml(evento);
		assertThat(xml).contains("evtBenPrRP");
		assertThat(xml).contains("<indRetif>1</indRetif>");
	}
	
	@Test
	public void deveGerarXmlBenPrRPRetificacao() throws Exception {
		Evento evento = getEvento();
		evento.getOcorrencia().setOperacao(Operacao.INCLUSAO);
		BenPrRP ocorrencia = (BenPrRP) evento.getOcorrencia().getDadosOcorrencia();
	...
	private Evento getEvento() throws Exception {
		...
		Ocorrencia ocorrencia = exemplo.lerOcorrencia(TipoOcorrencia.BENEFICIO_RPPS);
		...
		evento.setTipoEvento(TipoEvento.BENEFICIO_RPPS);
